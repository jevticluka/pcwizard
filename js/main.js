...
window.onload = function () {
  if (JSON.parse(localStorage.getItem("cart")) == null) {
    ...
    document.getElementById("numberOfProducts").textContent = JSON.parse(
  ...
  function filtriranjeBrendova(niz) {
    ...
    for (let i = 0; i < $(".brand:checked").length; i++) {
      odabraniBrendovi.push(
        parseInt($(".brand:checked")[i].value.substring(5))
      );
    }
  ...
  function filtriranjeKategorija(niz) {
    ...
    for (let i = 0; i < $(".category:checked").length; i++) {
      odabraneKategorije.push(
        parseInt($(".category:checked")[i].value.substring(8))
      );
    }
    ...
    for (let i = 0; i < niz.length; i++) {
      if (niz[i].id == proizvodID) {
        ...
        html += `<ul class="list">
                ...
                niz[i].inStock ? "In Stock" : "Unavailable"
              }</li>
						</ul>`;
        if (niz[i].availableSizes != null) {
          html += `<p>Available: </br>`;
          for (let j = 0; j < niz[i].availableSizes.length; j++) {
            html += `<input type="radio" class="pixel-radio size" name="size" value="${
              niz[i].availableSizes[j]
            }" id="${niz[i].name + niz[i].availableSizes[j]}"><label for="${
              niz[i].name + niz[i].availableSizes[j]
            }">${niz[i].availableSizes[j]}</label>`;
            html += `</br>`;
          }
  ...
  function prikazDostupnosti(niz) {
    ...
    document.getElementById("dostupnoText").textContent =
      "Available (" + br + ")";
    document.getElementById("nedostupnoText").textContent =
      "Unavailable (" + br1 + ")";
  }
  ...
  function dodajUkorpu() {
    ...
    function productExist() {
      return korpa.filter((x) => x.id == idProizvoda && x.size == velicina)
        .length;
    }
    ...
    if ($(".size:checked").length != 0) {
      for (let i = 0; i < $(".size").length; i++) {
        if ($(".size:checked")) {
          if (
            !document
              .getElementById("izaberiVelicinu")
        ...
        html += `<div class="col-md-6 col-lg-4 col-xl-3">
                <div class="card text-center card-product">
                  <div class="card-product__img">
                    ...
                    <img class="card-img" src="${niz[i].img.src}" alt="${
          niz[i].img.alt
        }">
                    </a>
                  </div>
        ...
        for (var j = 0; j < niz.length; j++) {
          if (niz[j].id == korpa[i].id) {
            if (i == 0) {
              ...
              html += `<div class="row p-5 korpaItem prviRedKorpa">
                        <div class="col-sm-6 col-md-4 col-lg-2 centar">
                            <img class="malaSlikaKorpa" src="../${
                              niz[j].img.src
                            }" alt="${niz[j].img.alt}"/></br>
                        </div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar">
                            <h5><a href="single-product.html?id=${niz[j].id}">${
                niz[j].name
              }</a></h5>
                        </div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar">
                            <span>Quantity: </span>
                            <button class="btn-plus" type="button" value="${
                              korpa[i].id
                            },${korpa[i].size},${
                korpa[i].qty
              }" name="btnQty"><img src="../img/plus.svg" alt="Plus svg"/></button>
                            <input class="kolicinaKorpa" type="number" value="${
                              korpa[i].qty
                            }" max="99" min="0" >
                            <button class="btn-minus" type="button" value="${
                              korpa[i].id
                            },${korpa[i].size},${
                korpa[i].qty
              }" name="btnQty"><img src="../img/minus.svg" alt="Minus svg"/></button>
                        </div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar"><span>Selected: ${
                          korpa[i].size
                        }</span></div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar"><span>Total price: ${
                          korpa[i].qty * niz[j].price.newPrice
                        }$</span></div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar">
                            <button class="removeItem button primary-btn" type="button" value="${
                              korpa[i].id
                            },${korpa[i].size},${korpa[i].qty}">Remove</button>
                        </div>
                        </div>`;
              break;
            }
            ...
            html += `<div class="row p-5 korpaItem">
                        <div class="col-sm-6 col-md-4 col-lg-2 centar"><img class="malaSlikaKorpa" src="../${
                          niz[j].img.src
                        }" alt="${niz[j].img.alt}"/></div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar"><h5><a href="single-product.html?id=${
                          niz[j].id
                        }">${niz[j].name}</a></h5></div>
                        <div class="col-sm-6 col-md-4 col-lg-2 centar">
                        <span>Quantity:</span>
  ...
  function UkloniProizvod() {
    ...
    if (dugmad != 1) {
      ...
      for (let i = 0; i < korpa.length; i++) {
        if (
          korpa[i].id == parseInt(dugme[0]) &&
          korpa[i].size == dugme[1] &&
          korpa[i].qty == parseInt(dugme[2])
        ) {
          korpa.splice(i, 1);
        }
      }
  ...
  function proveriEmail() {
    ...
    if (!uzorakEmail.test(email)) {
      ...
      if (email == "" || !email.trim()) poljeEmail.innerHTML = "Enter email!";
  ...
  function proveriGrad() {
    ...
    if (!uzorakGrad.test(grad)) {
      ...
      if (grad == "" || !grad.trim()) poljeGrad.innerHTML = "Enter city name";
  ...
  function ispisProizvodaCheckout(niz) {
    ...
    for (let i = 0; i < korpa.length; i++) {
      for (let j = 0; j < niz.length; j++) {
        if (korpa[i].id == niz[j].id) {
          suma += korpa[i].qty * niz[j].price.newPrice;
          html += `<li class="checkoutProizvodi"><a href="single-product.html?id=${
            korpa[i].id
          }">${niz[j].name} <span class="last"><span class="kolicinaKorpa">x ${
            korpa[i].qty
          }</span> &nbsp;&nbsp;&nbsp; ${
            korpa[i].qty * niz[j].price.newPrice
          }$</span></a></li>`;
    ...
    html += `<li><a href="javascript:void(0)">Subtotal <span>${suma}$</a></li>
                ...
                <li><a href="javascript:void(0)" class="pointer">Total <span>${
                  suma + 50
                }$</span></a></li>`;
